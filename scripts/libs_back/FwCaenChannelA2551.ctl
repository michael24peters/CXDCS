...
FwCaenChannelA2551_initialize(string domain, string device)
{
  fwDU_createGlobalIndex(domain, device);
  CaenLVLastAction[fwDU_getGlobalIndex(domain, device)] = "";
}

FwCaenChannelA2551_valueChanged( string domain, string device,
...
FwCaenChannelA2551_doCommand(string domain, string device, string command)
{ 
  string runType;
